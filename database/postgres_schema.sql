...
CREATE TABLE stock_transactions (
  ...
  gross_amount NUMERIC(20, 6) CHECK (gross_amount >= 0),
  fee NUMERIC(20, 6) CHECK (fee >= 0),
  tax NUMERIC(20, 6) CHECK (tax >= 0),
  net_amount NUMERIC(20, 6),
  cash_settlement_amount NUMERIC(20, 6) CHECK (cash_settlement_amount > 0),
  currency VARCHAR(3) NOT NULL CHECK (currency ~ '^[A-Z]{3}$'),
  note TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
...
COMMENT ON COLUMN stock_transactions.cash_settlement_amount IS '使用者實際填寫的資金戶入扣款金額；未填則由程式計算流水金額';
...
COMMENT ON COLUMN fund_transactions.cash_settlement_amount IS '使用者實際填寫的資金戶入扣款金額；未填則由程式計算流水金額';
